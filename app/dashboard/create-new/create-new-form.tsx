...
export function CreateNewLink() {
    ...
    const form = useForm<createLinkSchema>({
        resolver: zodResolver(createLinkSchema), defaultValues: {
    ...
    async function onValid(data: createLinkSchema) {
        const r = await fetchApi("/api/links", { method: "POST", body: JSON.stringify(data) })
        if (r.code == "success") {
            toast({ title: "Link has been shortened", description: "You will be redirected to link manage page." })
        }
    }

    return <form className="space-y-4" onSubmit={form.handleSubmit(onValid)}>
        <Form {...form}>
            ...
            <FormField control={form.control} name="slug" render={({ field }) => {
                return <FormItem>
                    ...
                    <div className="flex gap-6">
                        ...
                        <Button type="button" variant="secondary">Check Availability</Button>
                    </div>
                </FormItem>
            }} />
            <Card className="p-4">
                <FormField control={form.control} name="generateQrCode" render={({ field: { value, ...field } }) => {
                    return <FormItem>
                        <div className="flex items-center gap-2">
                            <FormLabel>Generate QR</FormLabel>
                            <FormControl>
                                <Switch  {...field} checked={value} onCheckedChange={field.onChange} />
                            </FormControl>
                        </div>
                        <FormDescription>Create a QR code along with link with customized colors and icon</FormDescription>
                    </FormItem>
                }} />

                {form.watch("generateQrCode") && <div className="my-4 flex items-center gap-x-4">
                    {/* Inputs */}
